...
//
// Getting object 
// {name john { algo {percent:90}}, {debugging {percent: 20} }
// list of activities
// list of groups
// list of objects, each one has activity id, user id. 
//
		
		$scope.allTasks = [ '.', 'algo', 'debugging', 'Logic', 'Loops', 'math', 'science', 'Loops', 'math', 'Loops', 'math','science']
		...
		$scope.studentNames = ['John', 'Mary', 'Mike'];


		$scope.students =
					[{name:'John', percentage:90, currentTask:'algo'},
	...
	//Matrix view Controllers
	$scope.selectedStudent = "Samir";
	$scope.switchPane = function(student){
	...
	$scope.findByUserAndActivity = function(userId, activity){
		var result = null;
		for (var index in this.stdTaskData){
			var item = this.stdTaskData[index];
			if (item.user == userId & item.task == activity){
				result = item;
			}
		}
		// this.currentPercent = item.percentage;
		return item;

	}

	$scope.dashProgressData = {};

	$scope.organizeData = function() {
		for (task in this.allTasks){
			var td = {};
			for (student in this.studentNames){
				// console.log(this.allTasks[task]);
				// console.log(this.studentNames[student]);
				var result = this.findByUserAndActivity(this.studentNames[student], this.allTasks[task]);
				td[this.studentNames[student]] = result.percentage;

		//		// console.log(student+task);
		  	}
		  	this.dashProgressData[this.allTasks[task]] = td;
		  	// alert("Asdasdasdas");
		}

		console.log(this.dashProgressData);
	}
